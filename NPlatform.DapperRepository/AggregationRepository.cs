...
namespace NPlatform.Repositories
{
    ...
    public  class AggregationRepository<TEntity, TPrimaryKey> : ResultBase, IAggregationRepository<TEntity, TPrimaryKey>
        where TEntity : AggregationBase<TPrimaryKey>
    {
        ...
        /// 仓储配置
        /// </param>
        public AggregationRepository(IRepositoryOptions option)
        {
            _Options = option;
        ...
        public int BulkLoad<T>(IEnumerable<T> datas)
        {
            if (!this.Options.ConnectSring.IsNullOrEmpty() && this.Options.DBProvider != default(DBProvider))
            {
                return new EPDBContext(this.Options.ConnectSring, this.Options.DBProvider).BulkLoad(datas);
            }
            else
            {
                return new EPDBContext(this.Options.ConnectName).BulkLoad(datas);
            }
        }
        ...
        public int BulkLoad(DataTable table)
        {
            if (!this.Options.ConnectSring.IsNullOrEmpty() && this.Options.DBProvider != default(DBProvider))
            {
                return new EPDBContext(this.Options.ConnectSring, this.Options.DBProvider).BulkLoad(table);
            }
            else
            {
                return new EPDBContext(this.Options.ConnectName).BulkLoad(table);
            }
        }
        ...
        public async Task<IEnumerable<TEntity>> GetListByExpAsync(
            Expression<Func<TEntity, bool>> filter,
            IList<Sort> sorts = null)
        {
            return await Task.Run<IEnumerable<TEntity>>(function: () => this.GetListByExp(filter, sorts));
        }
        ...
        /// <param name="sorts">排序</param>
        /// <returns><placeholder>A <see cref="Task"/> representing the asynchronous operation.</placeholder></returns>
        public async Task<IListResult<TEntity>> GetPagedAsync(
            int pageIndex,
            int pageSize,
            Expression<Func<TEntity, bool>> filter,
            IList<Sort> sorts)
        {
            var task = await Task.Run<ListResult<TEntity>>(
                           () =>
                               {
                                   ListResult<TEntity> result = new ListResult<TEntity>();
                                   var total = 0L;
                                   var rst = this.GetPaged(pageIndex, pageSize, filter, sorts);
                                   result.Data = rst.Data;
                                   result.Total = rst.Total;
                                   return result;
                               });
            return task;
        }
